...
public class TaobaoCrawler extends Crawler {
	...
	private static TaobaoCrawler taobaoCrawler;
...

    {
		...
		taobaoCrawler.productService = this.productService;
		
		taobaoCrawler.configService = this.configService;
		
		taobaoCrawler.shopService = this.shopService;
	...
	public void doStart(Shop shop) {
		...
		for (int i = 1; i < pageCount+1; i++) {
			...
			System.out.println("------Parser-----pageUrl-----"+pageUrl);
			start(pageUrl);
		}
				...
				if(productId!=null&&!productId.equals("")){					
					...
					if(prouductOrlPriceStr!=null&&!prouductOrlPriceStr.equals("")){
						...
						if(prouductSpecialPrice<priceMax){
							...
							existProduct.setShopCanal("taobao");
							List<Product> existProducts = taobaoCrawler.productService.queryProduct(existProduct);
							...
							List<Config> configs = taobaoCrawler.configService.queryConfig(config);
							if(configs!=null&&configs.size()>0){
								...
								taobaoCrawler.configService.update(con);				
							}else{
								continue;
							}
							...
							if(product!=null){
								taobaoCrawler.productService.save(product);
			...
			if(0==0){							
				...
				if(nameArray!=null){
					for(int i = 0;i<nameArray.length;i++){
						...
						List<Dict> dicts = taobaoCrawler.dictService.queryDict(dict);
						if(dicts.size()>0){			
							//保存数据字典
...
//					}
				...
				if (productImages != null) {
					...
					for (int i = 0; i < productImages.size(); i++) {
						...
						if (i == 0) {
							product.setImage("/tb/"+this.shop.getShopSku()+"/"+this.shop.getCategory()+"/"+ product.getSku() + "-" + (i + 1)
									+ ".jpg");
							product.setSmall_image("/tb/"+this.shop.getShopSku()+"/"+this.shop.getCategory()+"/"+ product.getSku() + "-" + (i + 1)
									+ ".jpg");
							product.setThumbnail("/tb/"+this.shop.getShopSku()+"/"+this.shop.getCategory()+"/"+ product.getSku() + "-" + (i + 1)
									+ ".jpg");
						}
						productMedia.setMedia_image("/tb/"+this.shop.getShopSku()+"/"+this.shop.getCategory()+"/"+ product.getSku() + "-" + (i + 1)
								+ ".jpg");
						...
						super.downloadFile(imageUrl, SystemConfig.Image_Path()+"/tb/"+this.shop.getShopSku()+"/"+this.shop.getCategory()+"/"+product.getSku() + "-"+ (i + 1)+ ".jpg");
						descriptionSb.append("<img src='{{media url='upload/tb/"+this.shop.getShopSku()+"/"+this.shop.getCategory()+"/"+ product.getSku() + "-" + (i + 1)+ ".jpg'}}' /><br/><br/>");
						imgNum = i+1;
					}				
				}
					...
					if (productColorImages != null) {
						...
						for (int i = 0; i < productColorImages.size(); i++) {
							...
							if(imageStr!=null&&!imageStr.equals("#")){
								...
								productMedia.setMedia_image("/tb/"+this.shop.getShopSku()+"/"+this.shop.getCategory()+"/" + product.getSku() + "-" + (imgNum + 1)+ ".jpg");
								...
								//HttpDownloadTool.download(imageUrl, 3,  productId + "-"+ (i + 1)+ ".jpg",this.category);
								//ImageDownloadTool.downLoadImage(imageUrl,  productId + "-"+ (i + 1)+ ".jpg",this.category);				
								super.downloadFile(imageUrl, SystemConfig.Image_Path()+"/tb/"+this.shop.getShopSku()+"/"+this.shop.getCategory()+"/"+product.getSku() + "-"+ (imgNum + 1)+ ".jpg");
								descriptionSb.append("<img src='{{media url='upload/tb/"+this.shop.getShopSku()+"/"+this.shop.getCategory()+"/"+ product.getSku() + "-" + (imgNum + 1)+ ".jpg'}}' />");
				...
				if(categoryOption.equals("Tops")||categoryOption.equals("Bottoms")){
					descriptionSb.append("<div style='clear:both;'></div><div><a name='size'></a><img src='{{media url='upload/tb/"+product.getShopSku()+"/"+product.getCategory()+"/"+ product.getSku() + "-size.jpg'}}' onerror=\"javascript:this.src='{{media url='upload/home/size.jpg'}}'\"/></div>");													
				}else if(categoryOption.equals("Shoes")){
					descriptionSb.append("<div style='clear:both;'></div><div><a name='size'></a><img src='{{media url='upload/tb/"+product.getShopSku()+"/"+product.getCategory()+"/"+ product.getSku() + "-size.jpg'}}' onerror=\"javascript:this.src='{{media url='upload/home/shoes-size.jpg'}}'\"/></div>");													
				}
	...
	public Product parseProduct(String productUrl,Product product) {
		...
		try {
			...
			if(0==0){	
				...
				StringBuilder descriptionSb = new StringBuilder();
				descriptionSb.append("<div>").append(productDescription);
				descriptionSb.append("<br/><br/><img  src='{{media url='upload/manual/"+product.getShopSku()+"/"+product.getCategory()+"/"+ product.getSku() + "-size.jpg'}}' /><br/><br/>");				
				descriptionSb.append("</div>");
				descriptionSb.append("<div style='clear:both;'></div>");
				descriptionSb.append("<div>");
				int imgNum = 0;
				...
				if(productColors != null){
					...
					if (productColorImages != null) {
						...
						for (int i = 0; i < productColorImages.size(); i++) {
							...
							ProductMedia productMedia = new ProductMedia();
							Element productImage = productColorImages.get(i);
							String imageStr= productImage.attr("href");
							if(imageStr!=null&&!imageStr.equals("#")){
								descriptionSb.append("<div>");		
								String imageUrl = imageStr.substring(imageStr.indexOf("(")+1, imageStr.indexOf(")")).replaceAll("30x30", "460x460");
