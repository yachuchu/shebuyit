...
public class ProductController extends BaseMultiActionController{	
	...
	private String productSearchView;
	
	private String productView;
	
	private String parserProductView;
	...
	public static String SEARCH_PARAM_ISEXPORT = "isExport";
	...
	public String getProductSearchView() {
		return productSearchView;
	}

	public void setProductSearchView(String productSearchView) {
	...
	public String getParserProductView() {
		return parserProductView;
	}

	public void setParserProductView(String parserProductView) {
		this.parserProductView = parserProductView;
	}
	...
	public ModelAndView search(HttpServletRequest request, HttpServletResponse response, DojoGridVO dojoGridVO) {
		...
		String searchParamFreeSize = request.getParameter(ProductController.SEARCH_PARAM_FREESIZE);
		String searchParamIsExport = request.getParameter(ProductController.SEARCH_PARAM_ISEXPORT);
		String paramStart = request.getParameter(ProductController.SEARCH_PARAM_START);		
		String paramEnd = request.getParameter(ProductController.SEARCH_PARAM_END);
		...
		if (searchParam != null){
			if (searchParam.equalsIgnoreCase(ProductController.SEARCH_PARAM_SKU)) {
				...
				queryCondition.append("&"+ProductController.SEARCH_PARAM+"="+ProductController.SEARCH_PARAM_SKU+"&"+ProductController.SEARCH_PARAM_VALUE+"=" + request.getParameter(ProductController.SEARCH_PARAM_VALUE));
			}else if(searchParam.equalsIgnoreCase(ProductController.SEARCH_PARAM_ORLSKU)) {
				product.setProductSku(request.getParameter(ProductController.SEARCH_PARAM_VALUE));
				queryCondition.append("&"+ProductController.SEARCH_PARAM+"="+ProductController.SEARCH_PARAM_ORLSKU+"&"+ProductController.SEARCH_PARAM_VALUE+"=" + request.getParameter(ProductController.SEARCH_PARAM_VALUE));
				...
				queryCondition.append("&"+ProductController.SEARCH_PARAM+"="+ProductController.SEARCH_PARAM_NAME+"&"+ProductController.SEARCH_PARAM_VALUE+"=" + request.getParameter(ProductController.SEARCH_PARAM_VALUE));
			}
			
		}
		if (searchParamShopsku != null){
				product.setShopSku(request.getParameter(ProductController.SEARCH_PARAM_SHOPSKU));				
				queryCondition.append("&"+ProductController.SEARCH_PARAM_SHOPSKU+"=" + request.getParameter(ProductController.SEARCH_PARAM_SHOPSKU));
			
		}
		if (searchParamCategory != null){
			product.setCategory(request.getParameter(ProductController.SEARCH_PARAM_CATEGORY));				
			queryCondition.append("&"+ProductController.SEARCH_PARAM_CATEGORY+"=" + request.getParameter(ProductController.SEARCH_PARAM_CATEGORY));
		...
		if (searchParamFreeSize != null&&!searchParamFreeSize.equals("")){
			...
			queryCondition.append("&"+ProductController.SEARCH_PARAM_FREESIZE+"=" + request.getParameter(ProductController.SEARCH_PARAM_FREESIZE));
		
		}
		if (searchParamIsExport != null&&!searchParamIsExport.equals("")){
			product.setIsExport(Integer.parseInt(request.getParameter(ProductController.SEARCH_PARAM_ISEXPORT)));				
			queryCondition.append("&"+ProductController.SEARCH_PARAM_ISEXPORT+"=" + request.getParameter(ProductController.SEARCH_PARAM_ISEXPORT));
		
		}		
		if (paramStart != null && !"".equals(paramStart)){
			product.setTime_start(paramStart);
			queryCondition.append("&"+ProductController.SEARCH_PARAM_START+"=" + request.getParameter(ProductController.SEARCH_PARAM_START));			
		}
		if (paramEnd != null && !"".equals(paramEnd)){
			product.setTime_end(paramEnd);
			queryCondition.append("&"+ProductController.SEARCH_PARAM_END+"=" + request.getParameter(ProductController.SEARCH_PARAM_END));			
		}
		dojoGridVO.getPaging().setQueryCondition(queryCondition.toString());
		ModelAndView mv = new ModelAndView(productSearchView);
		try {
	...
	public ModelAndView add(HttpServletRequest request, HttpServletResponse response, DojoGridVO dojoGridVO) {
		ModelAndView mv = new ModelAndView(parserProductView);
		Map categoryMap = DataUtils.categoryMap();
	...
	public ModelAndView parserProduct(HttpServletRequest request, HttpServletResponse response, DojoGridVO dojoGridVO) throws IOException {
		ModelAndView mv = new ModelAndView(parserProductView);
		...
		String attribute_set = DataDictionary.getAttributeSet(category);
		String returnCode ="000";
		Product product= new Product();
		product.setCategory(category);
		product.setAttribute_set(attribute_set);
		product.setShopSku(shopSku);
		if("sheinside".equals(shopCanal)){
			SheinsideCrawler sheinsideCrawler = new SheinsideCrawler();
			product = sheinsideCrawler.parseProduct(productUrl, product); 
		}else if("taobao".equals(shopCanal)){
			TaobaoCrawler taobaoCrawler = new TaobaoCrawler();	
			product = taobaoCrawler.parseProduct(productUrl, product);
		}else if("tianmao".equals(shopCanal)){
			TianmaoCrawler tianmaoCrawler = new TianmaoCrawler();
			product = tianmaoCrawler.parseProduct(productUrl, product);
		}else if("tianmaoJewelry".equals(shopCanal)){
			TianmaoJewelryCrawler tianmaoJewelryCrawler = new TianmaoJewelryCrawler();
			product = tianmaoJewelryCrawler.parseProduct(productUrl, product);
		}
		...
		if(product!=null){
			saveSuccessStr.append("{returnCode:'" + returnCode + "',productId:'" +product.getId()+ "'}");
		}else{
...
//		Map lengthMap = DataUtils.lengthMap();
	...
	public ModelAndView save(HttpServletRequest request, HttpServletResponse response, DojoGridVO dojoGridVO)  throws ServletException, IOException {				
		...
		return new ModelAndView("redirect:/product/update.do?productId="+product.getId()+"&returnCode="+returnCode);
	}
	...
	public ModelAndView exportSelectProduct(HttpServletRequest request, HttpServletResponse response, DojoGridVO dojoGridVO)  throws IllegalStateException, ServletException, IOException {				
		...
		if(productIds!=null&&!productIds.equals("")){	
			...
			while(productIterator.hasNext()){
				...
				String fileName =product.getShopSku()+"-"+product.getCategory().toLowerCase().replaceAll("/", "-")+"-Product-"+product.getSku();	
				File csvFile = CSVFile.createCSVFile("Category",productList, path, product.getShopSku()+"-"+product.getCategory().toLowerCase().replaceAll("/", "-")+"-Product-"+product.getSku());			
	...
	public ModelAndView exportProduct(HttpServletRequest request, HttpServletResponse response, DojoGridVO dojoGridVO)  throws IllegalStateException, ServletException, IOException, InterruptedException {				
		...
		String path = this.getServletContext().getRealPath("/");  
		String fileName =searchParamShopsku+"-"+searchParamCategory.toLowerCase().replaceAll("/", "-")+"-Product-"+time;
		
		File csvFile = CSVFile.createCSVFile(searchParamCategory,productList, path, fileName);			
	...
	public ModelAndView updateProducts(HttpServletRequest request, HttpServletResponse response, DojoGridVO dojoGridVO)  throws IllegalStateException, ServletException, IOException, InterruptedException {				
		...
		productSearch.setStock(1);
		if (searchParam != null){
			if (searchParam.equalsIgnoreCase(ProductController.SEARCH_PARAM_SKU)) {
				productSearch.setSku(request.getParameter(ProductController.SEARCH_PARAM_VALUE));								
			}else if(searchParam.equalsIgnoreCase(ProductController.SEARCH_PARAM_NAME)) {
				productSearch.setEnglishName(request.getParameter(ProductController.SEARCH_PARAM_VALUE));				
			}
		}
		if (searchParamShopsku != null){
			productSearch.setShopSku(request.getParameter(ProductController.SEARCH_PARAM_SHOPSKU));				
				
		}
		if (searchParamCategory != null){
			productSearch.setCategory(request.getParameter(ProductController.SEARCH_PARAM_CATEGORY));				
			
		}
		if (searchParamFreeSize != null&&!searchParamFreeSize.equals("")){
			productSearch.setIsFreeSize(Integer.parseInt(request.getParameter(ProductController.SEARCH_PARAM_FREESIZE)));				
			
		}
		if (paramStart != null && !"".equals(paramStart)){
			productSearch.setTime_start(paramStart);
			
		}
		if (paramEnd != null && !"".equals(paramEnd)){
			productSearch.setTime_end(paramEnd);			
		}
		List<Product> productList = productService.queryProduct(productSearch);
		int i =1;
		List<String> errorlist = new ArrayList<String>();
//		TianmaoJewelryCrawler tianmaoJewelryCrawler = new TianmaoJewelryCrawler();
		for(Product product:productList){
			System.out.println("-------------"+i+++"-------"+product.getId());

			String stock_url = product.getSb_stock_url();
		    String tem  =     stock_url.replaceAll("[^x00-xff]*", "");
		    product.setSb_stock_url(tem);
			productService.update(product);
	 ...
	 * @throws InterruptedException 
	 */
	public ModelAndView generatePrice(HttpServletRequest request, HttpServletResponse response, DojoGridVO dojoGridVO)  throws IllegalStateException, ServletException, IOException, InterruptedException {				
		String orl_priceStr = request.getParameter("orlPrice");
		String category = request.getParameter("category");
		String returnCode = "000";
		String specialPrice = "";
		String price = "";
		if(!orl_priceStr.equals("")&&!category.equals("")){
			double  orl_priceStr_double = Double.valueOf(orl_priceStr);
			double expressFee = DataDictionary.getExpressFee(category.replaceAll(" ", "_"));	
			if(orl_priceStr_double<=50){
				double special_price = (orl_priceStr_double*2.2+expressFee)/DataDictionary.getDollarRate();	
				specialPrice = String.format("%.2f",special_price);
				price = String.format("%.2f",special_price*1.5);
			}else if(orl_priceStr_double>50&&orl_priceStr_double<=100){
				double special_price = (orl_priceStr_double*2+expressFee)/DataDictionary.getDollarRate();	
				specialPrice = String.format("%.2f",special_price);
				price = String.format("%.2f",special_price*1.5);
			}else if(orl_priceStr_double>100&&orl_priceStr_double<=130){
				double special_price = (orl_priceStr_double*1.9+expressFee)/DataDictionary.getDollarRate();	
				specialPrice = String.format("%.2f",special_price);
				price = String.format("%.2f",special_price*1.5);
			}else if(orl_priceStr_double>130&&orl_priceStr_double<=160){
				double special_price = (orl_priceStr_double*1.8+expressFee)/DataDictionary.getDollarRate();	
				specialPrice = String.format("%.2f",special_price);
				price = String.format("%.2f",special_price*1.5);
			}else if(orl_priceStr_double>160&&orl_priceStr_double<=200){
				double special_price = (orl_priceStr_double*1.7+expressFee)/DataDictionary.getDollarRate();	
				specialPrice = String.format("%.2f",special_price);
				price = String.format("%.2f",special_price*1.5);
			}else if(orl_priceStr_double>200){
				double special_price = (orl_priceStr_double*1.6+expressFee)/DataDictionary.getDollarRate();	
				specialPrice = String.format("%.2f",special_price);
				price = String.format("%.2f",special_price*1.5);
			}
		}else{
		...
		saveSuccessStr.append("<html><head></head><body>");
		saveSuccessStr.append("<textarea>");
		saveSuccessStr.append("{returnCode:'" + returnCode + "',specialPrice:'" + specialPrice + "',price:'" + price + "'}");
		saveSuccessStr.append("</textarea>");
		saveSuccessStr.append("</body></html>");
		response.setCharacterEncoding("UTF-8");
		response.getWriter().print(saveSuccessStr.toString());
