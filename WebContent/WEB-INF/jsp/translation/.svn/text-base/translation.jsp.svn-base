...
$(document).ready(function(){
	...
	var validator = $("#translationform").validate({
		rules: {
			zhName: {
				required: true
			},
		...
		messages: {
			zhName: {
				required: "<fmt:message key='required.zhName' />"
			},
			enName: {
				required: "<fmt:message key='required.enName' />"
			},
		...
		submitHandler: function() {
						dojo.io.iframe.send({
			        		url: "<c:url value='/translation/save.do'/>",  
				       		form: dojo.byId("translationform"), 
				       		handleAs: "json",
				       		timeout:"15000",
				       		handle: function(response, ioArgs){
				       		if(response.returnCode == "000" || response.returnCode==000){	       					
				       			document.getElementById("successBox").style.display = "block";
			       			}else if(response.returnCode == "001" || response.returnCode==001){
			       				document.getElementById("errorBoxMesg").innerHTML="<fmt:message key='enName.exist' />";	
	...
	<div id="page">
		...
		<div class="wrapper">
				...
				<section class="column width6 first">					
					...
					<form id="translationform" method="post" action="<c:url value='/translation/save.do'/>">	
					        <p>
					        <label><fmt:message key='translation.translationId'/><fmt:message key='common.colon'/></label>
						    <input type="hidden" id="translationId" class="" value="${translation.id}" name="translationId"/>
						    ${translation.id}						    
						    </p>
						    
							<p>
							  <label class="required" for="zhName"><fmt:message key='translation.zhName'/><fmt:message key='common.colon'/></label><br/>
							  <input type="text" id="zhName" class="half" value="${translation.zhName}" name="zhName"/>
							</p>	
							
							<p>
							  <label class="required" for="enName"><fmt:message key='translation.enName'/><fmt:message key='common.colon'/></label><br/>
							  <input type="text" id="enName" class="half" value="${translation.enName}" name="enName"/>
							</p>
																																																													
							
						    <p class="box">
						    <input type="submit" class="btn btn-green big" value="<fmt:message key='button.submit'/>"/>&nbsp;&nbsp;&nbsp;&nbsp;						    
						    <input type="button" class="btn btn-green big" onclick="searchTranslation()" value="<fmt:message key='button.back'/>"/>
						    </p>
